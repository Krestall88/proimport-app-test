...
BEGIN
    ...
    SELECT
        gri.id as batch_id,
        (gri.quantity_received - COALESCE(rs.reserved_quantity, 0)) as available_qty
    FROM goods_receipt_items gri
    LEFT JOIN reserved_stock rs ON gri.id = rs.goods_receipt_item_id
    WHERE gri.product_id = p_product_id
      AND (gri.quantity_received - COALESCE(rs.reserved_quantity, 0)) >= p_required_qty -- Фильтруем партии, где достаточно товара
