-- Миграция для исправления отображения цены в заказах

-- Обновляем представление для заказов агента
...
-- Обновляем представление для инвентаря
...
SELECT 
    ...
    gri.expiry_date,
    gri.characteristics,
    SUM(gri.quantity_available) AS quantity,
    p.unit,
    -- Используем цену из goods_receipt_items, если она есть и больше 0, иначе из products
...
WHERE gri.quantity_available > 0
GROUP BY 
