-- Создание представления для страницы заказов менеджера с обеими ценами (исправленная версия)

-- Удаляем старое представление, если оно существует
DROP VIEW IF EXISTS manager_orders_view;

-- Создаем новое представление с обеими ценами
CREATE VIEW manager_orders_view AS
SELECT 
    co.id AS order_id,
    co.created_at,
    co.status,
    ...
    coi.quantity,
    p.unit,
    -- Закупочная цена только из products (в goods_receipt_items нет такого поля)
